    ...
    def get(self,lane_id,input):
                ...
                if overall_rating:
                    for key,val in overall_rating.items():
                        for re_key,re_val in sorted_loads.items():
                            if key == re_key:
                                val[settings.mismatch] = val[settings.mismatch]+re_val[settings.mismatch]
                                val[settings.percentage] = val[settings.percentage]+re_val[settings.percentage]
                                val[settings.count] = val[settings.count]+re_val[settings.count]
        ...
        if count > 1:
            for over_key,over_val in overall_rating.items():
                over_val[settings.percentage] = round(over_val[settings.percentage]/count)
